...
import { FaAlignJustify, FaChevronUp, FaCross } from "react-icons/fa";
    ...
    <div className="sm:block lg:hidden ">
      <header className=" flex text-white-soft p-4 px-10 mx-auto md:w-full max-w-7xl justify-between items-center text-center">
        ...
        <Drawer isOpen={isOpen} placement="top" onClose={onClose} finalFocusRef={btnRef}>
          <DrawerOverlay className="top-20">
                ...
                <VStack>
                  {navItems.map((navItem: NavItem, index: number) => (
                    <Link
                      key={index}
                      href={navItem.link}
                      className={`font-serif  text-2xl px-5 py-3.5 hover:text-gold w-full text-center${
                  ...
                  >
                    cancel <Icon as={FaChevronUp}></Icon>
                  </button>
                </VStack>
              </DrawerBody>
            </DrawerContent>
