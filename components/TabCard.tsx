  ...
  return (
    ...
    >
      ...
      <div className="hidden md:block w-3/5">
        <HStack spacing={0}>
          <p className="w-3/12 text-center text-lg tracking-tight whitespace-pre">
            {tab.tuning && tab.tuning.strings}
          </p>
          <p className="w-3/12 text-center">{tab.genre}</p>
          <div className="w-3/12">{tab.button && <Tab.Button button={tab.button} />}</div>
          <div className="w-3/12">
            <HStack justifyContent="flex-end" spacing={2}>
              <Tab.Social link={tab.guitardex} type="gdex" />
              <Tab.Social link={tab.spotifyLink} type="spotify" />
              <Tab.Social link={tab.videoLink} type="youtube" />
            </HStack>
          </div>
        </HStack>
      </div>
